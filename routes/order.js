...
router.post('/addcomment',fetchuser,[
    body('comments','Enter a valid commrtnt').isLength({ min: 3 }),
    body('sentiments','Enter a valid commrtnt').isLength({ min: 3 }),
],async(req,res)=>{
    try {
    const{comments,sentiments}=req.body;
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
    const comment= new Comment({
     user:req.user.id,comment:comments,sentiment:sentiments
    })
    const saveOrder=await comment.save()
